...
const Timetable = {
  ...
  async render(readOnly = false) {
    try {
      ...
      // Student Division & Batch Isolation Check
      const currentUser = (API.getSessionUser ? API.getSessionUser() : (typeof getSessionUser === 'function' ? getSessionUser() : null));
      const isStudent = (currentUser && currentUser.role === 'STUDENT');
      const db = (API.getLocalDB ? API.getLocalDB() : (typeof getLocalDB === 'function' ? getLocalDB() : { students: [] }));
      const studentProfile = App.currentProfile || (isStudent && currentUser ? db.students.find(s => s.user_id === currentUser.id) : null);

      if (isStudent && studentProfile) {
        if ((studentProfile.division_name && studentProfile.division_name.includes('-B')) || studentProfile.division_id == 2) {
          this.activeDivisionFilter = 'DIV_B';
        } else {
          this.activeDivisionFilter = 'DIV_A';
        }
      } else {
        if (!this.activeDivisionFilter) this.activeDivisionFilter = 'ALL';
      }
      ...
      const timeSlotsHeaderHtml = `
        <th>08.30 TO 09.30</th>
        <th>09.30 TO 10.30</th>
        <th style="width: 45px; background: #dbeafe; color: #1e3a8a;">SHORT BREAK</th>
        <th>10.40 TO 11.40</th>
        <th>11.40 TO 12.40</th>
        <th style="width: 45px; background: #dbeafe; color: #1e3a8a;">LUNCH BREAK</th>
        <th>01.30 TO 03.30</th>
      `;

      // Helper to render card HTML matching official timetable image
      const renderCard = (e) => {
      ...
      days.forEach((dayObj, dayIndex) => {
        ...
        targetDivisions.forEach((div, divIdx) => {
          ...
          const slotTds = timeSlots.map((slot) => {
            ...
            if (isStudent && studentProfile) {
              const bId = studentProfile.batch_id;
              const bName = studentProfile.batch_name;
              filteredEntries = slotEntries.filter(e => {
                if (!e.batch_id && !e.batch_name) return true;
                if (e.batch_name === "All Class") return true;
                if (bId && e.batch_id == bId) return true;
                if (bName && e.batch_name === bName) return true;
                return false;
              });
            }
            ...
            if (isStudent) {
              if (filteredEntries.length > 0) {
                const cardsHtml = filteredEntries.map(renderCard).join('');
                return `<td style="padding: 0.35rem;">${cardsHtml}</td>`;
              } else {
                return `<td style="padding: 0.35rem; text-align: center; color: var(--text-muted); font-size: 0.75rem; font-weight: 600;">-</td>`;
              }
            }
      ...
      return `
        <div class="dashboard-container">
          <div class="timetable-container">
            ...
            <!-- Course Details, Faculty In-Charge & Laboratory Legend -->
            <div style="margin-top: 2rem; display: grid; grid-template-columns: 2fr 1fr; gap: 1.5rem;">
              <!-- Course In-Charge Table -->
              <div class="glass-panel" style="background: #ffffff; padding: 1.25rem;">
                <h3 style="font-size: 1rem; color: #1e3a8a; margin-bottom: 0.75rem; border-bottom: 2px solid #e2e8f0; padding-bottom: 0.5rem; display: flex; align-items: center; gap: 0.5rem;">
                  <i class="fa-solid fa-book-open-reader"></i> Course Name, Abbreviation & Course In-Charge
                </h3>
                  ...
                  <table class="custom-table" style="font-size: 0.82rem;">
                    <thead>
                      <tr style="background: #f1f5f9;">
                        <th>Course Name</th>
                        <th style="width: 70px;">ABBR.</th>
                        <th>Course In-Charge (SE-A & SE-B)</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr>
                        <td><strong>Signals and Systems</strong></td>
                        <td><span class="badge" style="background: #dbeafe; color: #1e40af; font-weight: 800;">SS</span></td>
                        <td>Dr. Sagar Shinde (SS)</td>
                      </tr>
                      <tr>
                        <td><strong>Data Structures and Algorithms</strong></td>
                        <td><span class="badge" style="background: #dbeafe; color: #1e40af; font-weight: 800;">DSA / DS</span></td>
                        <td>Dr. Dhanashree Kulkarni (DK)</td>
                      </tr>
                      <tr>
                        <td><strong>Semiconductor Devices and Circuits</strong></td>
                        <td><span class="badge" style="background: #dbeafe; color: #1e40af; font-weight: 800;">SDC</span></td>
                        <td>Dr. Priyanka Patil (PP)</td>
                      </tr>
                      <tr>
                        <td><strong>Fundamental of Data Science</strong></td>
                        <td><span class="badge" style="background: #dbeafe; color: #1e40af; font-weight: 800;">FDS</span></td>
                        <td>Ms. Dhanashree Dixit (DD)</td>
                      </tr>
                      <tr>
                        <td><strong>Principles of Management and Entrepreneurship</strong></td>
                        <td><span class="badge" style="background: #dbeafe; color: #1e40af; font-weight: 800;">PME</span></td>
                        <td>Ms. Muktai Surnar (MS)</td>
                      </tr>
                      <tr>
                        <td><strong>Vocational and Skill Enhancement Course - I</strong></td>
                        <td><span class="badge" style="background: #dbeafe; color: #1e40af; font-weight: 800;">VSEC</span></td>
                        <td>Mr. Vikas T. / Ms. Sujata Gaikwad (SG) / Ms. Dhanashree Dixit (DD)</td>
                      </tr>
                      <tr>
                        <td><strong>Open Elective Course</strong></td>
                        <td><span class="badge" style="background: #dbeafe; color: #1e40af; font-weight: 800;">OEC</span></td>
                        <td>Mr. Vikas Gupta (VG)</td>
                      </tr>
                      <tr>
                        <td><strong>Value Added Course</strong></td>
                        <td><span class="badge" style="background: #dbeafe; color: #1e40af; font-weight: 800;">VEC</span></td>
                        <td>Visiting Faculty (VF)</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              ...
              <!-- Laboratory & Classroom Details -->
              <div class="glass-panel" style="background: #ffffff; padding: 1.25rem;">
                <h3 style="font-size: 1rem; color: #1e3a8a; margin-bottom: 0.75rem; border-bottom: 2px solid #e2e8f0; padding-bottom: 0.5rem; display: flex; align-items: center; gap: 0.5rem;">
                  <i class="fa-solid fa-flask-vial"></i> Laboratory & Location Details
                </h3>
                <div style="display: flex; flex-direction: column; gap: 0.65rem; font-size: 0.85rem;">
                  <div style="padding: 0.6rem; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: var(--radius-sm);">
                    <strong style="color: var(--primary);">Innovation & Design Laboratory:</strong><br>
                    <span style="font-weight: 700; color: #0f172a;">ID Lab-104</span>
                  </div>
                  <div style="padding: 0.6rem; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: var(--radius-sm);">
                    <strong style="color: var(--primary);">Advance Computing Laboratory:</strong><br>
                    <span style="font-weight: 700; color: #0f172a;">AC Lab-112</span>
                  </div>
                  <div style="padding: 0.6rem; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: var(--radius-sm);">
                    <strong style="color: var(--primary);">Semiconductor & Embedded Systems Lab:</strong><br>
                    <span style="font-weight: 700; color: #0f172a;">SES Lab-108</span>
                  </div>
                  <div style="padding: 0.6rem; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: var(--radius-sm);">
                    <strong style="color: var(--primary);">Theory Classrooms:</strong><br>
                    <span style="font-weight: 700; color: #0f172a;">Class Room-105 & Class Room-107</span>
                  </div>
                  <div style="padding: 0.6rem; background: #fffbeb; border: 1px solid #fde68a; border-radius: var(--radius-sm); font-size: 0.78rem; color: #92400e;">
                    <i class="fa-solid fa-circle-info"></i> <strong>Note:</strong> Students who opted for PCB will report to Lab No-104 & for Python programming to Lab No-112.
                  </div>
                </div>
              </div>
            </div>

            <!-- Department Signatories -->
            <div style="margin-top: 1.5rem; background: #ffffff; border: 1px solid #e2e8f0; border-radius: var(--radius-md); padding: 1.25rem; display: grid; grid-template-columns: repeat(4, 1fr); text-align: center; gap: 1rem;">
              <div>
                <strong style="color: #0f172a; font-size: 0.9rem; display: block;">Dr. Priyanka Patil</strong>
                <span style="font-size: 0.8rem; color: var(--text-muted); font-weight: 700;">DAC</span>
              </div>
              <div>
                <strong style="color: #0f172a; font-size: 0.9rem; display: block;">Dr. Dhanashree Kulkarni</strong>
                <span style="font-size: 0.8rem; color: var(--text-muted); font-weight: 700;">HoD (ECE)</span>
              </div>
              <div>
                <strong style="color: #0f172a; font-size: 0.9rem; display: block;">Dr. Sagar Shinde</strong>
                <span style="font-size: 0.8rem; color: var(--text-muted); font-weight: 700;">Dean Academics</span>
              </div>
              <div>
                <strong style="color: #0f172a; font-size: 0.9rem; display: block;">Dr. Pramod Patil</strong>
                <span style="font-size: 0.8rem; color: var(--text-muted); font-weight: 700;">Director</span>
              </div>
            </div>
          </div>
        </div>
  ...
  async handleDeleteEntry(entryId) {
    ...
    try {
      await API.del(`/api/timetable/${entryId}`);
